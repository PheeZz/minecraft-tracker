...
После ванильного + модового разрешения:
- **`recipes.json`**: ~**70 %** ссылок на предметы названы (229 выходов), стоимости аспектов — 100 %.
- **`aspect-sources.json`**: **260/320** item-субъектов названы.
Не названы: предметы с под-типами по мете (одно поле = много имён, нужна мета для выбора, напр.
`NetherShard`/`itemShard`/`singularity`) и ссылки из циклов/локальных переменных.
