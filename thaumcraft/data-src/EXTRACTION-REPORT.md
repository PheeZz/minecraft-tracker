...
### Материалы и палочки  →  `materials.json`
**24** материала инструментов (harvestLevel/durability/efficiency/damage/enchantability),
**15** материалов брони (durabilityFactor/damageReduction[шлем,нагрудник,поножи,ботинки]/enchantability)
по всем модам, плюс **6** наверший палочек (`visDiscount`/`craftCost`) и **9** стержней
(`visCapacity`/`craftCost`).
...
8. AE2: позиции в дереве + **аспекты исследований** извлечены (8/11; разобраны Feature-классы);
   у 3 базовых записей аспектов нет по дизайну.
